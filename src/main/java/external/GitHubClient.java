...
public class GitHubClient {
	// Basic templet of Gitbut API url parameter
	private static final String URL_TEMPLATE = "https://jobs.github.com/positions.json?description=%s&lat=%s&long=%s";
	private static final String DEFAULT_KEYWORD = "developer";  // default keyword of job search
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		// encode the string to convert all special character to url format. 
		try {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() != 200) { // Status is not correct.
			...
			// return a input stream. The response might be long so it need several data. 
			BufferedReader reader = new BufferedReader( new InputStreamReader(entity.getContent())); // default buffer size: 8kb
